...
namespace StudentAcademicManagement.Api.Controllers
{
    ...
    public class StudentProfilesController : ControllerBase
    {
        ...
        public StudentProfilesController(IStudentProfileService profileService, ApplicationDbContext context)
        {
        ...
        public async Task<IActionResult> GetProfile(int studentId)
        {
            try
            {
                ...
                return Ok(await _profileService.GetProfileByStudentIdAsync(schoolId.Value, studentId));
            }
        ...
        public async Task<IActionResult> GetMyProfile()
        {
            try
            {
                ...
                return Ok(await _profileService.GetProfileByStudentIdAsync(student.SchoolId, student.Id));
            }
            catch (Exception ex) { return BadRequest(new { message = ex.Message }); }
        }

        [HttpPut("{studentId}")]
        public async Task<IActionResult> UpdateProfile(int studentId, [FromForm] UpdateStudentProfileRequest request)
        {
            try
            {
                var schoolId = await GetSchoolIdAsync();
                ...
                return Ok(await _profileService.UpdateProfileAsync(schoolId.Value, studentId, request));
            }
            catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
            catch (Exception ex) { return BadRequest(new { message = ex.Message }); }
        }
